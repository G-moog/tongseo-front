...
export default function DailyRoutinesWidget() {
  ...
  return (
    <div className="px-4 pt-2 pb-1 shrink-0">
      <div className="bg-[#151400] border border-[#302e00] rounded-xl overflow-hidden">
        {/* 헤더 */}
        <div className="px-3 py-2 border-b border-[#302e00] flex items-center justify-between">
          <span className="text-xs font-semibold text-yellow-600/90 tracking-wider">Daily Routines</span>
          <span className={`text-[10px] font-medium ${allDone ? 'text-yellow-400' : 'text-[#4a4700]'}`}>
          ...
          {todayRoutines.map((routine, i) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 hover:bg-[#1e1c00] active:bg-[#252300] transition-colors text-left ${
                  i < todayRoutines.length - 1 ? 'border-b border-[#302e00]' : ''
                }`}
              >
                {/* 체크박스 */}
                <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-all ${
                  checked
                    ...
                    : 'border-[#504d00] bg-transparent'
                }`}>
                  {checked && (
                    <svg className="w-2.5 h-2.5 text-[#151400]" fill="none" viewBox="0 0 10 10">
                ...
                <span className={`text-sm transition-all ${
                  checked ? 'line-through text-[#3a3800]' : 'text-yellow-100/80'
                }`}>
                  {routine.title}
                </span>
              </button>
